...
**Seegent = 项目管理指挥部**，不是 AI 执行引擎。

- Seegent 管理项目、追踪进度、组织文件
- 写代码/做图/批量自动化由外部智能体（WorkBuddy、Cursor）完成
- 内置 AI 的职责：分析数据、总结进展、整理文件、生成周报
- 不再追求 Agent 增强（Shell、并行调用等）——那是"干活"用的，不是"管理"用的
...
- 社媒数据监测功能已确认不做，前端已无此功能
- README.md：删除了"数据监测（社媒查询）"一行
- 删除了 `.seegent-data-platforms.json`（社媒监测配置残留）
- **项目看板确认是核心功能**：拾元老师每天早上第一件事就是打开看板看数据/待办/开始工作
  - Seegent-PRD.md：看板从"待实现"提升为 P0 优先，"已实现"也标记了 V1 已存在
  - README.md：加回了"项目看板"作为核心模块
  - 附录：看板文档改为"✅ 核心特性"
  - 路线图：看板排在最前（现在→一个月 第1项）
- 项目工作流追踪保留为"待决定开发方向"
