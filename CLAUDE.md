...
| CLI | `.partnerfm-cli.json` | 已连接 CLI 工具展示（Cursor Agent、Hermes Agent） |
| MCP | `.partnerfm-mcp.json` | MCP CLI 服务器管理（CRUD + 启用/禁用 + 工具发现 + Agent 集成） |
| 角色 | `.partnerfm-roles.json` | 系统角色管理（单选 + CRUD），聊天时作为 system prompt 注入 |

> 社交平台接入（如飞书）走 MCP 模块：在 `DEFAULT_MCP`（server.py）+ `.partnerfm-mcp.json` 中维护预置条目，命令里留 `<your_app_id>`/`<your_app_secret>` 占位符，用户编辑卡片填入真实凭证后即可被 Agent 调用。新平台只需在这两处加条目，无需改 agent/前端。
...
- **事故**：`.partnerfm-models.json` 含明文 DeepSeek API Key，commit `d2aa6d5`（2026-06-11 14:07）push 到公开 GitHub 仓库，key 被第三方盗用，6/11-6/12 余额耗尽
- **修复**：
  - `git filter-branch` 清除所有历史中的敏感配置文件 + force push
  - `.gitignore` 新增所有 `.partnerfm-*.json`，防止再次误提交
  - `server.py` 新增 `DEEPSEEK_API_KEY` 环境变量兜底：`api_key = req.get('api_key', '') or DEEPSEEK_API_KEY`
  - `.partnerfm-models.json` 中 key 字段留空，真实密钥不再存文件
...
- 用户配置任意 MCP CLI 命令（飞书、ima 知识库、Notion 等），PartnerFM 自动管理子进程并转换工具为 Agent 可调用格式
...
- 启用/禁用开关持久化到 `.partnerfm-mcp.json`
...
### 第 1 阶段不做的（留给第 2 阶段）
- ❌ OCR（图片内文字建索引）— 环境：`PIL` + `pytesseract` 已装，但 `tesseract` 命令未装
- ✅ ~~向量语义检索~~ → 已在 2026-06-15 完成（见下方）
- ❌ PDF/Word 全文建索引（`pdfplumber`/`python-docx` 未装，LibreOffice 已装可转 PDF）
- 环境：Python 3.9.6，`numpy`/`openpyxl`/`requests` 已装
...
- 配置文件 `.partnerfm-embedding.json`：`{ provider, api_key, base_url, model, dimensions }`
- `_embed_texts(texts)` 工具函数：批量调 `/embeddings` 端点，内存 LRU 缓存（200 条）
- 配置 UI 在「模型管理」模块底部

### SQLite 向量索引
- 数据库：`.partnerfm-index.db`（WAL 模式，单进程无并发问题）
...
### 维护注意
- `.partnerfm-index.db` 和 `.partnerfm-embedding.json` 已加入 `.gitignore`
...
| 功能 | 说明 |
|------|------|
| 飞书 MCP 接入 | `@larksuiteoapi/lark-mcp` 真实命令，`DEFAULT_MCP` + `.partnerfm-mcp.json` 同步 |
| 删除 filesystem MCP | 写死桌面、冗余，替换为「本地文件」内置工具展示卡 |
| 文件夹路径绑定 | `bindFolderPath` + `folder.path` + 右键重绑 + 恢复时回填 |
| search_files 升级 | 子串匹配 → 行号+上下文段落，最多 50 文件 |
| read_file 分段读 | offset/limit 参数，长文件不丢失 |
| _exec_tool 重复执行 bug | 修复工具执行两遍 |
| 多模态消息通道 | 贴图/拖图/粘贴 → base64 → OpenAI vision 数组 |
| 多对话独立打断 | `chatAbortMap` 按 sessionId 隔离 |
| embedding 配置 + SQLite 向量索引 | `.partnerfm-index.db`，500 字分块，增量更新 |
| 3 个新 agent 工具 | `semantic_search` / `file_stats` / `recent_files` |
| 侧边栏索引图标 | ⚪未索引 / 🔵已索引 / 🟡过期，点击触发 reindex SSE |
| 🔪 砍掉群聊功能 | 删除 server.py 中 GROUP_CHAT_FILE、_group_chat()、_call_llm()、群聊端点；删除 index.html 中群聊 CSS/HTML/JS（~550 行） |
| OCR 图片建索引 | 新增 IMAGE_EXTS 常量、_iter_image_files()、_ocr_image()（pytesseract），_index_workspace() 支持图片 OCR 文本索引（无 embedding） |
| PDF/Word 全文索引 | 安装 pdfplumber/python-docx，新增 _extract_pdf_text()/_extract_docx_text()/_extract_doc_text()/_read_file_content()，TEXT_EXTS 扩展 .pdf/.docx/.doc |
| 企业微信 MCP | DEFAULT_MCP 新增 wecom 条目，参照飞书模式配置占位符和教程 |

### 架构备忘（截至 2026-06-15 关键数据流）

```
用户添加文件夹 → bindFolderPath（绑路径）
    → folder.path → /api/workspaces 持久化
    → resolveAutoWorkspace(session) → workspace 传给 /api/agent
    → agent 拥有 9 个工具（含 semantic_search/file_stats/recent_files）
...
用户配置数据平台 → /api/data-platforms 存磁盘
    → 前端选平台 tab（抖音/小红书/...）+ 搜索/热门
    → POST /api/data-proxy {channel, action, params}
    → _call_data_api(cfg) → URL 拼接 + auth header 转发
    → 返回 JSON → 前端渲染结果卡片
```

## 📊 数据监测模块（2026-06-15 新增）
- 左侧导航新增 📊 监测按钮
- **通用平台适配**：默认 RedFox，改配置即可换数据源
- 后端 `_call_data_api(cfg, action, channel, params)` 集中做 URL 拼接 + auth 转发
- `DATA_ACTION_PATHS` 映射 action→URL 路径，换平台只改这里
- 结果卡片通用渲染：字段自适应（title/name/nickname + followers/likes + avatar）
- 配置 `DEFAULT_DATA_PLATFORMS.channels` 预置 10 个社媒平台
